...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect check_bb2(pos, pos + ImVec2(square_sz, square_sz));
	ImRect total_bb(check_bb);

	//total_bb.Max += ImVec2(0, 4);
	ItemSize(total_bb);
    if (!ItemAdd(total_bb, id))
        return false;

	ImGuiIO& io = ImGui::GetIO();
	ImVec2 siz = io.Fonts->Fonts[0]->CalcTextSizeA(13.f, FLT_MAX, 0.0f, label);


	//total_bb.Max -= ImVec2(0, 4);
	total_bb.Max += ImVec2(siz.x + style.ItemInnerSpacing.x, 0 );
    ...
    ImU32 check_col = GetColorU32(ImGuiCol_Highlight);
    if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue) {
		...
		window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 1, check_bb.Min.y + 1), ImVec2(check_bb.Max.x - 1, check_bb.Max.y - 1), check_col);
	}

    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb2.Max.x + style.ItemInnerSpacing.x, check_bb2.Min.y - 2.0f), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
	frame_bb.Max = frame_bb.Max - ImVec2(2, 0);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 1), grab_bb.Max + ImVec2(-1, 1), GetColorU32(g.ActiveId == id ? ImGuiCol_Highlight : ImGuiCol_Highlight), style.GrabRounding);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

	ImGui::PushFontDropShadow(0xFF000000);
	RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
	ImGui::PopFontDropShadow();
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
		...
		ImVec2 pos = ImVec2(bar0_pos_x, bar0_line_y);
		//ImVec2 pos2 = ImVec2(bar0_pos_x, bar0_pos_y + 3);
		ImVec2 fasdf = ImVec2(bar0_pos_x - 1, bar0_line_y) + ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz);
		draw_list->AddRectFilled(pos + ImVec2(-1, 0), pos + ImVec2(20, 1), IM_COL32(255, 255, 255, 255 / 1));
		//draw_list->AddRectFilled(ImVec2(0, 0), ImVec2(100,100), IM_COL32(0, 0, 0, 255));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    /*if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
    {
        ...
        SameLine();
        TextEx("Color");
        EndDragDropSource(); //if you try to drag and drop one of the two preview icons on the right into the color picker it crashes
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    {
		const float separator_min_x = tab_bar->BarRect.Min.x - 0.5;// - IM_FLOOR(window->WindowPadding.x );
		const float separator_max_x = tab_bar->BarRect.Max.x - 0.5;// + IM_FLOOR(window->WindowPadding.x );
...
#include <Windows.h>
void ImGui::Keybind(const char* name, unsigned int* Key) {
	static bool setKey = false;
	static bool firstTime = true;
	char keyBuffer[4096];
	char key[64];
	unsigned int scanCode = MapVirtualKeyA(*Key, MAPVK_VK_TO_VSC);
	GetKeyNameTextA(scanCode << 16, key, 64);

	sprintf_s(keyBuffer, 4096, "%s: %s", name, key);
	const char* keyButton = keyBuffer;
	if (setKey)
		sprintf_s(keyBuffer, 4096, "Setting %s", name);
	if (ImGui::Button(keyButton)) {
		setKey = !setKey;
		firstTime = true;
	}
	if (setKey) {
		static BYTE keyboardArray[256];
		static BYTE oldKeyboardArray[256];
		GetKeyboardState((PBYTE)&keyboardArray);
		for (int i = 0x0; i < 256; i++) {
			if (firstTime)
				break;
			if (keyboardArray[i] != oldKeyboardArray[i]) {
				setKey = false;
				*Key = i;
				break;
			}
		}
		memcpy(oldKeyboardArray, keyboardArray, 256);
		firstTime = false;
	}
}
//-------------------------------------------------------------------------
