...
typedef enum PlayerConditions_t
{
...
typedef enum PlayerLifeStates_t
{
	LIFE_ALIVE = 0,
	LIFE_DEAD = 2
} PlayerLifeStates;
...
typedef enum WeaponProficiency_t
{
...
typedef enum class ConvarFlags_t
{
...
typedef struct CClientClass_t
{
	char			_chPadding[sizeof(uintptr_t) * 2];
	char *chName;
	void *Table; //RecvTable*
	CClientClass_t *pNextClass;
	int				iClassID;
} CClientClass;

typedef struct CUserCmd_t
{
	void*		vtable;
	int			CommandNumber;
	int			TickCount;
...
typedef struct WeaponProficiencyInfo_t
{
	float spreadscale;
	float bias;
...
class CBoneCache{
public:
	float timeValid; //0x0000
	uint32_t boneMask; //0x0004
	uint32_t size; //0x0008
	uint16_t cachedBoneCount; //0x000C
	uint16_t matrixOffset; //0x000E
	uint16_t cachedToStudioOffset; //0x0010
	uint16_t boneOutOffset; //0x0012

	inline matrix3x4* BoneArray() {
		return (matrix3x4 *)((char *)(this + 1) + matrixOffset);
	}

	inline short *StudioToCached()
	{
		return (short *)((char *)(this + 1));
	}

	inline short *CachedToStudio()
	{
		return (short *)((char *)(this + 1) + cachedToStudioOffset);
	}
	inline void UpdateBones(const matrix3x4 *pBoneToWorld, int numbones, float curtime)
	{
		matrix3x4 *pBones = BoneArray();
		const short *pCachedToStudio = CachedToStudio();

		for (int i = 0; i < cachedBoneCount; i++)
		{
			int index = pCachedToStudio[i];
			memcpy(pBones[i], pBoneToWorld[i], sizeof(matrix3x4));
		}
		timeValid = curtime + 1;
	}
};
...
public:
	...
	CGameTrace() { }
	CGameTrace(const CGameTrace& balls) {
		m_fFractionLeftSolid = balls.m_fFractionLeftSolid; m_Surface = balls.m_Surface; m_iHitGroup = balls.m_iHitGroup; m_pEntity = balls.m_pEntity;  m_iHitbox
			= balls.m_iHitbox;
	}
...
#pragma endregion "Empty Classes"
